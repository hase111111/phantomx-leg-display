...
class GraphDisplayer:
    ...
    def display(self, hexapod_pram = HexapodParam(), *, 
        ...
        if display_table:
            ...
            self._ax_table = self._fig.add_subplot(3,3,2)   # 今回は使用しないので適当な座標に配置.
        ...
        hexapod_leg_power = HexapodLegPower(
            hexapod_calc, hexapod_pram, self._fig, self._ax, 
        ...
        app_graph = ApproximatedGraphRenderer(
            hexapod_calc, self._ax, z_min=Z_MIN, z_max=Z_MAX,
        ...
        # 脚を描画.
        self.leg_renderer = HexapodLegRenderer(hexapod_calc, hexapod_pram, self._fig, self._ax, self._ax_table, display_circle=display_circle, display_wedge=display_wedge)
        self.leg_renderer.set_img_file_name(image_file_name)
        self.leg_renderer.render()

        # マウスがグラフのどこをポイントしているかを示す線を描画する.
        self.mouse_grid_renderer = MouseGridRenderer(self._fig, self._ax, alpha=alpha_mouse_grid, color=color_mouse_grid)
        if display_mouse_grid:
            self.mouse_grid_renderer.render()

        # 脚の可動範囲を描画する.
        hexapod_range_of_motion = HexapodRangeOfMotion(
            hexapod_calc, hexapod_pram, self._ax,
